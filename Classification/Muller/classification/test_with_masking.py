...
from classification.dataset_with_masking import FiveCropImageDataset
...
segment_class_idx_all = np.array([0]) #np.array([3,2,5,1,14,13,17,18,7,10,12,22,35,33,21,27,4,44])
for segment_class_idx in segment_class_idx_all:           
    dataloader = [] 
    print('PROCESSING SEMANTIC SEGMENTATION CLASS: ' + str(segment_class_idx))    

    for image_dir, meta_csv in zip(args.image_dirs, args.meta_files):
        dataset = FiveCropImageDataset(meta_csv, image_dir, segment_class_idx)
        dataloader.append(
    ...
    fout = Path(args.checkpoint).parent / ("test-" + "_".join(
        [str(Path(x).stem) for x in args.image_dirs]) + str(segment_class_idx) +".csv")
    print("Write to", fout)
    df.to_csv(fout)
