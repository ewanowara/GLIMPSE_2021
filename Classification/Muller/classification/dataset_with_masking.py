...
import scipy.io
import numpy as np
from classification.segmentation_masking import mask_segmented_object
...
class MsgPackIterableDatasetMultiTargetWithDynLabels(torch.utils.data.IterableDataset):
    ...
    def _process_sample(self, x):
        ...
        # Ewa TODO mask each class idx - provided at the begining of testing bc the test will be ran separately for each of 150 classes
...
class FiveCropImageDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        segment_class_idx: Union[int, Path],
        ...
        self.meta_info = None
        self.segment_class_idx = segment_class_idx
    ...
    def __getitem__(self, idx) -> Tuple[torch.Tensor, dict]:
        ...
        # ###### Ewa: semantic segmentation masking #####
        img_RGB_seg = np.array(Image.open(meta["img_path"]).convert("RGB"))
        if segment_class_idx == 0: # don't segment
            image = img_RGB_seg # then input should be original images without segmentation
        else:
            image = mask_segmented_object(img_RGB_seg, self.segment_class_idx)
            # image.save("tmp/" + str(meta[self.img_id_col]) + "img_masked.jpg") # masked
